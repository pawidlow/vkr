...
plt.plot(logTime, veloc3, label=r'$\dot{\phi}$', color='k')
plt.xlabel('t, s')
plt.grid()
plt.legend()
plt.show()
...
#p.setJointMotorControlArray(bodyIndex=boxId, jointIndices=jointIndices, targetPositions=[0.1, -1.6665, qd], controlMode=p.POSITION_CONTROL)
for _ in range(1000):
    p.stepSimulation()
    #time.sleep(dt)
